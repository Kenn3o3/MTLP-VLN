...
parser.add_argument("--action_repeat", default=40, type=int, help="Number of simulation steps to repeat each action.")
...
class Planner:
    def __init__(self, env, env_cfg, args_cli, simulation_app):
        ...
        self.action_queue = Queue()  # Queue for user actions in training mode
        ...
        # Add reverse mapping for inference
        self.idx_to_action = {0: "Move forward", 1: "Turn left", 2: "Turn right"}
        # Velocity commands corresponding to action indices
        ...
        # end visualization ###
        # Initialize and load the policy model
    ...
    def run_episode(self, episode_idx: int) -> None:
        ...
        current_instruction = self.env_cfg.instruction_text  # Use full instruction
        ...
        if self.args_cli.train:
            # Training mode: User provides actions via web UI
            while self.step_counter < self.max_steps and not done:
                ...
                # Update UI to indicate waiting for action
                ...
                self.log_manager.log_step(
                    self.step_counter, self.robot_path, bgr_image_np, depth_map, "Waiting for action",
                ...
                action = self.action_queue.get()  # Blocks until action is received
                print(f"User selected action: {action}")
                # Collect initial state with the chosen action (before stepping)
                if self.step_counter % self.k_steps == 0:
                    instructions.append(current_instruction)
                    rgbs.append(rgb_image_np.copy())
                    depths.append(depth_map.copy())
                    actions.append(action)
                # Action received, proceed with execution
            ...
            # Inference mode: Model predicts actions automatically
            hidden = None
            robot_pos = self.env.unwrapped.scene["robot"].data.root_pos_w[0].cpu().numpy()
            rgb_image_np = infos['observations']['camera_obs'][0, :, :, :3].clone().detach().cpu().numpy().astype(np.uint8)
            depth_map = infos['observations']['depth_obs'][0].cpu().numpy().squeeze()
            while self.step_counter < self.max_steps and not done:
                ...
                for _ in range(self.k_steps):
                    ...
                    if goal_distance < 1.0:
                        ...
                        break
                hidden = new_hidden  # Update hidden state for next prediction
        ...
        print(status)
        # Train the policy if in training mode
        if self.args_cli.train:
            # Save training data
    ...
    def start_loop(self):
        ...
        if self.args_cli.train:
            print("Running in training mode with user input.")
